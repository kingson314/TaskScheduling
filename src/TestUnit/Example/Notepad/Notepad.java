...
public class Notepad extends JFrame implements ActionListener, MouseListener,
	...
	private JCheckBoxMenuItem checkstatuJMenuitem;
	...
	public Notepad() {
		...
		this.getContentPane().add(new JScrollPane(text)); // TextField加滚动条
		...
		// 加弹出菜单
		text.addMouseListener(this); // TextField中加鼠标监听器
		mypopupmenu();
		text.add(popupmenu); // 把弹出菜单加在TextField中
	...
	private void MyToolbar() {
		...
		pnlpicture.add(btn);
		btn.addActionListener // 监听接口在( ）内实现s
				(new ActionListener() {
					public void actionPerformed(ActionEvent event) {
						...
						try {
							...
							// public ImageIcon(String filename) 根据指定的文件创建一个
							// ImageIcon
							ImageIcon icon = new ImageIcon("30.jpg");
							text.insertIcon(icon);
							text
									.setCaretPosition(position >= insert ? position + 1
											: position);
	...
	private void mypopupmenu() // 弹出菜单
	{
		...
		popupmenuitem[2].setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_X,
				InputEvent.CTRL_MASK)); // 剪切快捷方式ctrl+X
		popupmenuitem[3].setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_C,
				InputEvent.CTRL_MASK));
		popupmenuitem[4].setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_V,
				InputEvent.CTRL_MASK));
		popupmenuitem[5].setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_A,
				InputEvent.CTRL_MASK));
	}
	...
	private void Editmenu() {
		String menuitemstr[] = { "撤消", "恢复", "剪切", "复制", "粘贴", "删除", "查找",
				"查找下一个", "替换", "转到", "全选", "时间/日期" };
		JMenuItem menuitem[] = new JMenuItem[menuitemstr.length];
		for (int i = 0; i < menuitemstr.length; i++) {
			menuitem[i] = new JMenuItem(menuitemstr[i]);
			menuitem[i].addActionListener(this);
	...
	private void watchmenu() {
		checkstatuJMenuitem = new JCheckBoxMenuItem("查看状态栏");
		menu[3].add(checkstatuJMenuitem);
		checkstatuJMenuitem.addActionListener(this);// 状态栏监听器
	}
	...
	public void writeToFile(String lines) // 将字符串lines写入到当件文本文件中
	{
		try {
			...
			JOptionPane.showMessageDialog(this, "有IO错，写入" + file.getName()
					+ "文件不成功");
		}
	}
	...
	public String readFromFile() // 将当前文本文件中的内容读出，以lines返回
	{
		...
		try {
			...
			JOptionPane.showMessageDialog(this, "" + this.file.getName()
					+ "文件不存在");

		} catch (IOException ioex) {
			JOptionPane.showMessageDialog(this, "IO错，读入" + file.getName()
					+ "文件不成功");
		} finally {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand() == "新建") {
			...
			this.text.setText("");
				...
				&& fchooser.showOpenDialog(this) == JFileChooser.APPROVE_OPTION)
		...
		else if ((e.getActionCommand() == "保存" && file == null || e
				.getActionCommand() == "另存为")
		...
		// 显示保存（空文件）或点击另存为按扭，出现保存对话框，点击确定（点击确定后会返回JFileChooser.APPROVE_OPTION
		{
			...
			text.selectAll();
		else if (e.getActionCommand() == "时间/日期") // 在TextArea后追加显示系统当前时间
		{
			Calendar time = Calendar.getInstance();
			String str = "    " + time.HOUR_OF_DAY + ":" + time.MINUTE + "  "
			...
			if (doc != null) {
				try {
					// void insertString(int offset, String str, AttributeSet
					// a)throws BadLocationException插入内容字符串。
			...
			panelstatus.setVisible(checkstatuJMenuitem.isSelected());
		} else if (e.getActionCommand() == "帮助主题") {
			JOptionPane
					.showMessageDialog(this,
							"To do by yourself,don't to depend on others\n你被耍了,hahaha……");
		} else if (e.getActionCommand() == "关于记事本") {
			JOptionPane.showMessageDialog(this,
					"this is made by yaing,liujian and liuli,2010-6-13");
		}
	}
	...
	public void caretUpdate(CaretEvent e) // 状态栏上显示光标所在的行号和列号
	{
		try {
			...
			text_status.setText("字付总数:" + pos + "  当前位置  Line:" + posLine
					+ "  Column:" + col);
		} catch (Exception ex) {
