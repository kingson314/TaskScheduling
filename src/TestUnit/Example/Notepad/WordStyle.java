...
//功能:修改字体、字形、字号，TextArea的前景色、背景色、对齐方式（没实现），以及用TextArea present预浏当前设置效果
...
public class WordStyle extends JFrame implements ActionListener {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -6031193037976478651L;
	private SComboBox wordname, wordstyle, wordsize; // 组合框
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == underline) {
			...
			doc.setParagraphAttributes(text.getSelectionStart(), text
					.getSelectionEnd()
					- text.getSelectionStart(), bSet, false);
		}
		if (e.getSource() instanceof SComboBox) {
			try {
				...
				StyledDocument doc = text.getStyledDocument();
				doc.setCharacterAttributes(text.getSelectionStart(), text
						.getSelectionEnd()
						- text.getSelectionStart(), aSet, false);

			} catch (NumberFormatException nfe) {
				JOptionPane.showMessageDialog(this, (String) wordsize
						.getSelectedItem()
						+ "不能转化成整数");
			} catch (Exception ec) {
