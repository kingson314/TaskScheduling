...
public class LogLatelyTable {
	private JTable jtable;

	// 构造
	public LogLatelyTable(Map<String, String> map, Map<String, AddLogLately> map_addLogLately) {
		try {
			...
			Vector<?> tableValue = LogLatelyDao.getInstance().getLogLatelyVector(map, map_addLogLately);
			int[] cellEditableColumn = null;
			int[] columnWidth = new int[] { 60, 120, 80, 220, 60, 120, 120, 80, 80, 120, 120 };
			int[] columnHide = null;
			...
			jtable.getColumn(columnName.get(2)).setCellRenderer(dcr);
		} catch (Exception e) {
	...
	private void mouseDoubleClick() {
		try {
			...
			if (jtable.getValueAt(jtable.getSelectedRow(), 5) == null)
				...
				execresult = execresult + "开始时间  :" + jtable.getValueAt(jtable.getSelectedRow(), 5).toString() + "\n";
			if (jtable.getValueAt(jtable.getSelectedRow(), 6) == null)
				execresult = execresult + "结束时间  :" + "\n";
			else
				execresult = execresult + "结束时间  :" + jtable.getValueAt(jtable.getSelectedRow(), 6).toString() + "\n";

			if (jtable.getValueAt(jtable.getSelectedRow(), 4) != null)
				execresult = execresult + "执行时间  :" + jtable.getValueAt(jtable.getSelectedRow(), 4).toString() + "\n";
			...
			if (jtable.getValueAt(jtable.getSelectedRow(), 7) == null)
				...
				execresult = execresult + "任务组标志:" + jtable.getValueAt(jtable.getSelectedRow(), 7).toString() + "\n";
			if (jtable.getValueAt(jtable.getSelectedRow(), 8) == null)
				execresult = execresult + "任务组名称:" + "\n";
			else
				execresult = execresult + "任务组名称:" + jtable.getValueAt(jtable.getSelectedRow(), 8).toString() + "\n";
			String taskId = "";
			if (jtable.getValueAt(jtable.getSelectedRow(), 9) == null)
				execresult = execresult + "任务标志  :" + "\n";
			else {
				taskId = jtable.getValueAt(jtable.getSelectedRow(), 9).toString();
			...
			if (jtable.getValueAt(jtable.getSelectedRow(), 10) == null)
				...
				taskName = jtable.getValueAt(jtable.getSelectedRow(), 10).toString();
				execresult = execresult + "任务名称  :" + taskName + "\n";
			}
			if (jtable.getValueAt(jtable.getSelectedRow(), 11) == null)
				execresult = execresult + "任务类型  :" + "\n";
			else
				execresult = execresult + "任务类型  :" + jtable.getValueAt(jtable.getSelectedRow(), 11).toString() + "\n";
			if (jtable.getValueAt(jtable.getSelectedRow(), 2) == null)
				execresult = execresult + "执行状态  :" + "\n";
			else
				execresult = execresult + "执行状态  :" + jtable.getValueAt(jtable.getSelectedRow(), 2).toString() + "\n";
			if (jtable.getValueAt(jtable.getSelectedRow(), 3) == null)
				execresult = execresult + "执行结果  :" + "\n";
			else
				execresult = execresult + "执行结果  :" + jtable.getValueAt(jtable.getSelectedRow(), 3).toString() + "\n";
