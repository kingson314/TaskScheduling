...
/**
 * @Description:中国招投标公共服务平台-变更公告公示
 * @date Aut 1,o19
 * @author:fgq
 */
public class TaskChange extends TaskAbstract {
	...
	private final static String cls="cebpubservice Change";
	...
	private int exec(Bean bean) throws Exception {
		...
		for (int i = 1; i <= bean.getPageIndex(); i++) {
			...
			String url = "http://bulletin.cebpubservice.com/xxfbcmses/search/change.html?searchDate="
					+ UtilConver.dateToStr(Const.fm_yyyy_MM_dd)
					+ "&dates=300&word=&categoryId=89&industryName=&area=&status=&publishMedia=&sourceInfo=&showStatus=&page="
					+ i;
...
//					if(info.getName().indexOf("知识")<0 && info.getName().indexOf("产权")<0 )continue;
						...
						info.setProvince("");
						info.setCity("");
						info.setRegion(tr.select("td:eq(1)").first().getElementsByTag("span").first().attr("title"));
						info.setUnit(getUnit(tr.select("td:eq(2)").html()));
						info.setSource("");
						info.setPublishTime(tr.select("td:eq(3)").html());
						info.setNameOriginal(tr.select("td:eq(4)").first().getElementsByTag("a").first().attr("title"));
						info.setMemo(tr.select("td:eq(4)").first().getElementsByTag("a").first().attr("href")
