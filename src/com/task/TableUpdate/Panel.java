...
public class Panel implements ITaskPanel {
	...
	private STextField txtDstCompareKey;
	private STextField txtSrcCompareKey;
	...
	private boolean paramValidate(Bean bean) {
		...
		if ("".equals(bean.getDstSelectSql())) {
			ShowMsg.showWarn("目标表SQL不能为空");
			return false;
		}
		if ("".equals(bean.getSrcCompareKey())) {
			ShowMsg.showWarn("源表关键字不能为空");
			return false;
		}
		if ("".equals(bean.getDstCompareKey())) {
	...
	public boolean fillTask(ITask task) {
		try {
			Bean bean = new Bean();
			bean.setDstCompareFields(UtilString.isNil(txtDstCompareFields.getText()));
			bean.setDstCompareKey(UtilString.isNil(txtDstCompareKey.getText()));
			bean.setDstDbName(UtilString.isNil(txtDstDbName.getText()));
			bean.setDstInsertSql(UtilString.isNil(txtrInsertSql.getText()));
			bean.setDstSelectSql(UtilString.isNil(txtrDstSelectSql.getText()));
			bean.setDstUpdateSql(UtilString.isNil(txtrUpdatetSql.getText()));
			bean.setSrcCompareFields(UtilString.isNil(txtSrcCompareFields.getText()));
			bean.setSrcCompareKey(UtilString.isNil(txtSrcCompareKey.getText()));
			bean.setSrcDbName(UtilString.isNil(txtSrcDbName.getText()));
			bean.setSrcSelectSql(UtilString.isNil(txtrSrcSelectSql.getText()));
			bean.setIfInsertWhenDifferent(rbtnInsert.isSelected());
			bean.setIfInsertWhenNotExist(chkInsert.isSelected());
			bean.setIfUpdateWhenDifferent(rbtnUpdate.isSelected());
	...
	public void fillComp(ITask task) {
		try {
			...
			txtDstCompareFields.setText(bean.getDstCompareFields());
			txtDstCompareKey.setText(bean.getDstCompareKey());
			txtDstDbName.setText(bean.getDstDbName());
			txtrInsertSql.setText(bean.getDstInsertSql());
			txtrDstSelectSql.setText(bean.getDstSelectSql());
			...
			txtSrcCompareFields.setText(bean.getSrcCompareFields());
			txtSrcCompareKey.setText(bean.getSrcCompareKey());
			rbtnInsert.setSelected(bean.isIfInsertWhenDifferent());
			rbtnUpdate.setSelected(bean.isIfUpdateWhenDifferent());
			chkInsert.setSelected(bean.isIfInsertWhenNotExist());
		} catch (Exception e) {
	...
	private STextField getTxtSrcCompareKey() {
		if (txtSrcCompareKey == null) {
			txtSrcCompareKey = new STextField();
			txtSrcCompareKey.setBounds(127, 182, 160, 21);
		}
		return txtSrcCompareKey;
	}

	private STextField getTxtDstCompareKey() {
		if (txtDstCompareKey == null) {
			txtDstCompareKey = new STextField();
			txtDstCompareKey.setBounds(407, 182, 160, 21);
		}
		return txtDstCompareKey;
	}
