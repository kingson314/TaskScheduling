...
public class Task extends TaskAbstract {
	// public static String[] Mode_Type = new String[] {"0","5","6","1", "2" ,
	// "3", "4"};
	// public static String[] Mode_TypeName = new String[] {"全部","破位式(升)"
	// ,"破位式(降)", "一升式", "一降式" ,"二升式", "二降式"};
	private Connection con;
	private Bean bean;
	private String begDate;
	private String endDate;

	private String price_symbol5_detail = " price_symbol5_detail ";
	private String price_symbol60_detail = " price_symbol60_detail ";
	private String price_symbol5 = " price_symbol5 ";
	private String price_symbol60 = " price_symbol60 ";
	private String sa_feature_symbol = " sa_feature_symbol ";
	private String sa_kline_symbol = " sa_kline_symbol ";
	...
	public void manuExecTask(String[] params) {
		this.begDate = params[0];
		this.endDate = params[1];
		this.fireTask();
	}

	public void fireTask() {
		this.bean = (Bean) JSONObject.toBean(JSONObject.fromObject(this.getJsonStr()), Bean.class);
		this.price_symbol5_detail = price_symbol5_detail.replace("symbol", this.bean.getSymbol());
		this.price_symbol60_detail = price_symbol60_detail.replace("symbol", this.bean.getSymbol());
		this.price_symbol5 = price_symbol5.replace("symbol", this.bean.getSymbol());
		this.price_symbol60 = price_symbol60.replace("symbol", this.bean.getSymbol());
		this.sa_feature_symbol = sa_feature_symbol.replace("symbol", this.bean.getSymbol());
		this.sa_kline_symbol = sa_kline_symbol.replace("symbol", this.bean.getSymbol());
		try {
			...
			init();
			String sql60 = "select dateServer,timeServer,dateLocal,timeLocal,open,close,high,low,ma5,ma20,ma60,kdj,id,type from " + price_symbol60
					+ "  where  dateserver>=? and dateserver<=? order by dateServer ,timeServer ";
	...
	private void init() throws Exception {
		String sql = "delete  " + sa_kline_symbol + "  where id60 in(select id from " + price_symbol60 + " where dateserver>=? and dateserver<=?) ";
		UtilSql.executeUpdate(this.con, sql, new Object[] { this.begDate, this.endDate });
		sql = " delete  " + sa_kline_symbol + "  where id60 in(select id from " + price_symbol60 + " where dateserver>=? and dateserver<=?)";
		UtilSql.executeUpdate(this.con, sql, new Object[] { this.begDate, this.endDate });
		sql = "update " + price_symbol60 + " set type=''  where dateserver>=? and dateserver<=? ";
		UtilSql.executeUpdate(this.con, sql, new Object[] { this.begDate, this.endDate });
	}

	private List<HashMap<String, String>> getList5(String dateTime60) throws Exception {
		String sql5 = "select dateServer,timeServer,dateLocal,timeLocal,open,close,high,low,ma5,ma20,ma60,kdj,id " + "from " + price_symbol5_detail + "  where dateServer>=? and dateServer<= ?"
				+ " and concat(dateServer,' ',timeServer) > ? and  concat(dateServer,' ',timeServer) <=? order by dateServer ,timeServer ";
	...
	private String downSymbolType(List<HashMap<String, String>> list60, int index) {
		...
		if (Double.valueOf(list60.get(index - 1).get("CLOSE")) > Double.valueOf(list60.get(index - 1).get("OPEN"))) {// 阳线
			if (Double.valueOf(list60.get(index).get("LOW")) < Double.valueOf(list60.get(index - 1).get("LOW"))) {
				type = "6";
				updateSymbolType(type, list60.get(index).get("ID"));
			}
		}
		return type;
	}
	 ...
	 *            ：k线的id号
	 */
	private void updateSymbolType(String type, String id) {
		Object[] param = new Object[2];
		param[0] = type;
		param[1] = id;
		String sql = "update " + price_symbol60 + " set type=? where id=? ";
		try {
	...
	private void updateFeatureKLine(Map<String, String> mapParam) {
		...
		param[0] = mapParam.get("ID60");
		param[1] = mapParam.get("MAXID");
		param[2] = mapParam.get("MINID");
		param[3] = mapParam.get("BADID");
		param[4] = mapParam.get("TYPE");
		String sql = "insert into " + sa_kline_symbol + " ( id60, id5max,id5min, id5bad,type)values(?,?,?,?,?)";
		try {
			UtilSql.executeUpdate(con, sql, param);
		} catch (Exception e) {
			e.printStackTrace();
	...
	private void updateFeatureStatistics(Map<String, String> mapParam) {
		...
		param[0] = mapParam.get("ID60");
		param[1] = mapParam.get("VALUEMAX");
		param[2] = mapParam.get("VALUEMIN");
		param[3] = mapParam.get("VALUEMAXMIN");
		param[4] = mapParam.get("VALUEMAXSUPER");
		param[5] = mapParam.get("TIMEMAX");
		param[6] = mapParam.get("TIMEMIN");
		param[7] = mapParam.get("TIMEBAD");
		String sql = "insert into " + sa_feature_symbol + "(id60,valuemax,valuemin,valuemaxmin,valuemaxsuper,timemax,timemin,timebad) values(?,?,?,?,?,?,?,?)";
		try {
			UtilSql.executeUpdate(con, sql, param);
		} catch (Exception e) {
			e.printStackTrace();
